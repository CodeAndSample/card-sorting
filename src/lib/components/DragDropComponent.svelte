<script>
	...
	function drop(event, basketIndex) {
		...
		const [item] = baskets2[data.basketIndex].items.splice(data.itemIndex, 1);

		// Add the item to the drop target basket.
		baskets2[basketIndex].items.push(item);
		baskets2 = baskets2;

		hoveringOverBasket = null;
		// console.log('Basket-Store', $baskets);
	}
	$: baskets2 = $baskets;
</script>
...
{#each baskets2 as basket, basketIndex (basket)}
	<div animate:flip>
		<b>{basket.name}</b>
		<ul
